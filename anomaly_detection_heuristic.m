function  [ypred, score, forecast] = anomaly_detection_heuristic(Time, p)
%ANOMALY_DETECTION_HEURISTIC  Detekce událostí v časové řadě stavu zaplnění nádob na papír či rychle se plnících nádob.
%
%   [ypred, score, forecast] = ANOMALY_DETECTION_HEURISTIC(Time, p)
...
for i = 51 : len-50 %Last 50 samples can be postprocessed within Golemio with a delay
    ...
    d1 = (p(i)-p(i-1))/hours((Time(i)-Time(i-1)));%Derivaition
    c1 = d1>10;
    c2 =  d2<-9;
    if all([c1 c2])
        %lab=[lab 6];
