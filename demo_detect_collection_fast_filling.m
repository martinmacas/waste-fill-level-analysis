%demo_detect_collection_fast_filling.m
% 
% The demonstration of the detection of quickly filling waste (paper) collections.
...
for fileIdx = 3:numel(files)
    ...
    %% Detect schedule change points and stable intervals
    [schedules, idxStart, idxEnd, centers, operates, collectionsRefined] = ...
        detect_changepoints(timeAll, fillLevelAll, collectionsInitial);
    ...
    for intervalIdx = 1:numel(idxStart)
        ...
        isLongEnough = analysis(intervalIdx).durationDays > 30 && ...
                       analysis(intervalIdx).numSamples > 100;
        ...
        [cSupervised, ~, ~, ~] = detect_collections_supervised( ...
            timeInterval, fillInterval, [], schedules{intervalIdx});
        ...
        if ~isempty(schedules{intervalIdx})
            [missingIdx, scheduledTimes] = detect_missing_collection( ...
                timeInterval, fillInterval, cSupervised, schedules{intervalIdx}, 12);
        end
        ...
        plot(timeInterval(cThresh), fillInterval(cThresh), 's', ...
            'MarkerFaceColor', [0.9290 0.6940 0.1250], 'MarkerSize', 10);
        ...
        if ~isempty(schedules{intervalIdx})
            ...
            % Plot schedule lines
            plot([scheduledTimes scheduledTimes]', ...
                repmat([0 100], numel(scheduledTimes), 1)', ...
    ...
    if ~isempty(missingIdx)    
        legend('Fill level signal', 'Thresholding', 'Supervised classifier','Missing collections', 'Scheduled collection');
    else
        legend('Fill level signal', 'Thresholding', 'Supervised classifier','Scheduled collection');
    end
